...
CREATE POLICY "Anyone can view active trucks" ON public.food_trucks FOR SELECT USING (is_active = true);
CREATE POLICY "Owners can manage their own truck" ON public.food_trucks FOR ALL USING (auth.uid() = owner_id) WITH CHECK (auth.uid() = owner_id);
...
CREATE POLICY "Truck owners can manage their menu" ON public.menu_items FOR ALL USING (
...
CREATE POLICY "Truck owners can manage their schedule" ON public.schedules FOR ALL USING (
